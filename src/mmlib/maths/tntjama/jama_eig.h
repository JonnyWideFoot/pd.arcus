			...
			for (int l = 0; l < n; l++) {
				...
				if (m > l) {
					...
					do {
						...
						Real g = d[l];
						Real p = (Real)((d[l+1] - g) / (2.0 * e[l]));
						Real r = (Real)hypot(p,1.0);
						if (p < 0) {
							r = -r;
						}
						d[l] = e[l] / (p + r);
			...
			while (n >= low) {
				...
				if (l == n) {
					...
					if (iter == 10) {
						...
						s = (Real)(abs(H[n][n-1]) + abs(H[n-1][n-2]));
						x = y = 0.75f * s;
						w = -0.4375f * s * s;
					}
