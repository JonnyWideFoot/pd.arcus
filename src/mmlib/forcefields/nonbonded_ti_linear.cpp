...
#include "forcefields/nonbonded_ti_linear.h" // provides base class
...
namespace Physics
{
	...
	template <
		...
		int T_VerboseMode
	>
	void FF_NonBonded_TI_Linear_CalcForces_T_fast2(
	FF_NonBonded_TI_Linear      &ff, 
	...
	struct FF_NonBonded_TI_Linear_CalcForces_T_fast2_wrap{
		FF_NonBonded_TI_Linear_CalcForces_T_fast2_wrap():ptr(&FF_NonBonded_TI_Linear_CalcForces_T_fast2<a, b, c, d>){}
		typedef void (*Tptr)(
			FF_NonBonded_TI_Linear      &ff, 
			NonBonded_Pack    *local_atomparam,
			Maths::dvector    *basisvector,
		...
		{
			throw(CodeException("CODE ERROR: FF_NonBonded_TI_Linear_CalcForces_T_fast2_wrap template requested that is out of bounds.") ); 
		}
	};

	void FF_NonBonded_TI_Linear::calcEnergies()
	{ 
		setupBasisVectors();
		calcForces();
	}

	void FF_NonBonded_TI_Linear::calcEnergiesVerbose(ForcefieldBase::AtomicVerbosity level)
	{ 
		...
		tcall4<FF_NonBonded_TI_Linear_CalcForces_T_fast2_wrap>
	...
	void FF_NonBonded_TI_Linear::calcForces(){ 
		setupBasisVectors();
