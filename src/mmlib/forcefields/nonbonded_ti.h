...
/// \bug BUGS?
///
	class PD_API FF_NonBonded_TI_SoftCore : public FF_NonBonded,
																					public FF_Extension_TI
	{
	public:
		FF_NonBonded_TI_SoftCore( WorkSpace &newwspace ):
		...
		~FF_NonBonded_TI_SoftCore(){
		};

		/// Sets the softness of the Softcore potential. Default is 9 
		double TI_Delta_Vdw;

		/// Sets the softness of the Softcore potential. Default is 8 
		double TI_Delta_Elec;
		
		/// Determines if the atoms fully decouple or only partially decouple.  False by default.
		bool FullyDecouple;
		...
		void info() const{ // prints a little block of parameter information
			...
			printf("INFO: TI_Delta_Vdw:             %6.3lf\n", TI_Delta_Vdw  );
			printf("INFO: TI_Delta_Elec:            %6.3lf\n", TI_Delta_Elec  );
			printf("INFO: FullyDecouple:            %s\n", FullyDecouple ? "Yes\n" : "No\n");
		}
...
/// \bug BUGS?
///
	class PD_API NonBonded_TI: public NonFF_Bonded_TemplateBoundary< T_Space >, public FF_Extension_TI
	{
	public:
		NonBonded_TI( WorkSpace &newwspace ) : 
			NonFF_Bonded_TemplateBoundary< PeriodicBox >( newwspace ),
			FF_Extension_TI()
		{
			settodefault();
		}
		...
		{ 
			...
			NonFF_Bonded_TemplateBoundary< T_Space >::info(); // call base class info function
			// add some more info
	...
	protected:
		...
		void settodefault()
		{
			NonFF_Bonded_TemplateBoundary< T_Space >::settodefault(); // call base class info function
		...
		virtual void calcEnergiesVerbose(ForcefieldBase::AtomicVerbosity level)
		{ 
			typedef  NonFF_Bonded_TemplateBoundary< T_Space > Base;
...
/// \bug BUGS?
///
	class PD_API NonBonded_TI_Zacharias: public NonFF_Bonded_TemplateBoundary< T_Space >,
		public FF_Extension_TI
	{
	public:
	...
	protected:
		...
		virtual void setup(){
			typedef  NonFF_Bonded_TemplateBoundary< T_Space > Base;
			NonFF_Bonded_TemplateBoundary< T_Space >::setup();

			const WorkSpace &wspace = Base::getWSpace();

			setupAtomGroups(wspace.nAtoms());
