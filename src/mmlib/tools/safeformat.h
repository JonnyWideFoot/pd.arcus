...
#ifndef __LOKI_SAFEFORMAT_H
#define __LOKI_SAFEFORMAT_H
...
namespace LokiCore
{
	...
	// We can also write to the MMLib StringBuilder as a 'Device'
...
#pragma warning(pop)
	...
	LokiCore::PrintfState<T&, Char> XPrintf(T& device, const Char* format) {
		return LokiCore::PrintfState<T&, Char>(device, format);
	}

	template <class T>
	LokiCore::PrintfState<T&, char> XPrintf(T& device, const std::string& format) {
		return LokiCore::PrintfState<T&, char>(device, format.c_str());
	}

	template <class Char, std::size_t N>
	LokiCore::PrintfState<std::pair<Char*, std::size_t>, Char> 
	BufPrintf(Char (&buf)[N], const Char* format) {
		std::pair<Char*, std::size_t> temp(buf, N);
		return LokiCore::PrintfState<std::pair<Char*, std::size_t>, Char>(temp, format);
	}
}// namespace LokiCore
