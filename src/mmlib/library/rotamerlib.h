...
namespace Library
{
	...
	enum RotamerRepresentation
	{
		Cartesian = 0,
		Torsional = 1
	};

	/// \brief Adding "Original" to the RotamerRepresentation
	enum AddWriteLibMode
	{
		Original = 2
	};

	/// Inherit an enum
	typedef InheritEnum< RotamerRepresentation, AddWriteLibMode > WriteLibMode;
	...
	public:
		...
		Rotamer() 
		{
			m_SourceRepresentation = Cartesian; // we have to initialise to something ...
		...
		inline RotamerRepresentation getSource() const { return m_SourceRepresentation; } ///< Where the 'ding-dang-doodly' did this rotamer come from?
	...
	private:
		...
		RotamerRepresentation m_SourceRepresentation; ///< Was this rotamer originally defined as a cartesian or torsional state?
	};
	...
	public:
		...
		void writeLib( const std::string& _FileName, WriteLibMode _mode = Original ); ///< Write a PD format library
