...
for i in $( du | grep -v "\.svn" | sed 's/.*\././' | grep "./" ); do
	...
	echo lib"$DIR"_la_SOURCES = $(ls *.cpp *.h  -d 2> /dev/null 2> /dev/null)            >> Makefile.am
	## add other shit
	echo 'INCLUDES = -I@top_srcdir@/src/mmlib  -I@top_srcdir@/src/mods/arcus' >> Makefile.am
...
SUBDIRS=$(du --max-depth=1 | grep -v "\./\." | grep "\./" | sed 's/.*\.\///' | grep -v _purg )
echo SUBDIRS = $SUBDIRS                                                   >> Makefile.am 
echo lib"$DIR"_la_SOURCES = $(ls *.cpp *.h  -d 2> /dev/null  2> /dev/null)            >> Makefile.am
echo 'INCLUDES = -I@top_srcdir@/src/mmlib'                                 >> Makefile.am

## and we need to find and add the names of all the convenience libraries
echo lib"$DIR"_la_LIBADD = $(du | grep -v "/\." | grep "\./" | grep -v _purg | sed 's/.*\.\///' | sed 's/\w*$/&\/lib&.la/')    >> Makefile.am
