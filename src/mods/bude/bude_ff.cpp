...
namespace Physics
{
	...
	void BudeForcefield::setup()
	{
		// Proxies
		WorkSpace& wspace = getWSpace();
		size_t natom = wspace.nAtoms();   ///< get the number of atoms in the workspace once in advance to save calculating it each loop
			...
			if( m_Cutoff_elec_formal >= 0 )
			{
				if( m_Cutoff_elec_partial >= 0 )
				{
					if( (m_LigandIndex >= 0) && (m_ReceptorIndex >= 0)
						...
						&& (m_LigandIndex < _wspace.nMolecules()) && (m_ReceptorIndex < _wspace.nMolecules()) )
					{
						return;
					}
					else
